...
export const Products = {
  ...
  async save(id, data) {
    const ref = id ? doc(db,'products',String(id)) : doc(collection(db,'products'));
    await setDoc(ref, {...data, updatedAt:Date.now()}, {merge:true});
    return ref.id;
  },
  async delete(id) { await deleteDoc(doc(db,'products',String(id))); },
  onSnapshot(cb) {
    return onSnapshot(collection(db,'products'), s => cb(s.docs.map(d=>({id:d.id,...d.data()}))));
  }
};
...
export const Orders = {
  async add(data) {
    ...
    const num        = counter.exists() ? (counter.data().next||1) : 1;
    await setDoc(counterRef, {next: num+1}, {merge:true});
    const ref = await addDoc(collection(db,'orders'), {
      ...data, orderNumber: num, timestamp: serverTimestamp(), status: 'new'
    });
...
export const FCMTokens = {
  async save(uid, token, role) {
    await setDoc(doc(db,'fcm_tokens',uid), {token, role, updatedAt:Date.now()}, {merge:true});
  }
};
...
export const OfflineQueue = {
  KEY: 'spark_offline_q',
  push(order) {
    const q = this.getAll(); q.push({...order, _at:Date.now()});
    localStorage.setItem(this.KEY, JSON.stringify(q));
  },
  getAll() { try { return JSON.parse(localStorage.getItem(this.KEY)||'[]'); } catch { return []; } },
  clear()  { localStorage.removeItem(this.KEY); },
  async flush() {
    const pending = this.getAll(); if (!pending.length) return 0;
    let sent = 0;
    for (const o of pending) { try { await Orders.add(o); sent++; } catch {} }
